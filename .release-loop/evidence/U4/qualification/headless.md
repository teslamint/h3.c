...
- Pre-state: compiled model exists; stdin closed
- Exact injection/command: `qualification test CLI < /dev/null`
- Exit status: `0`
...
- Post-state: result and receipt written without prompt
- Next invocation / compensation result: same flags rerun deterministically
- Mechanism check: closed stdin and structured JSON prove headless contract
