...
| 7 | in-progress | P1 | Make the one-command conversion gate rerunnable with legacy default work state | On merged `main`, exact `H3_ANE_TRACE=1 make h3_ane_integration_test` exits 2 because an older `/private/tmp/h3-ane-integration` lacks the current ownership marker. A fresh isolated override still passes package generation, compilation, production metadata/schema loading, and inventory `441/149/149/0` with null parity/receipt. Pass requires the exact default command to handle or safely migrate supported legacy state without weakening ownership checks. |
